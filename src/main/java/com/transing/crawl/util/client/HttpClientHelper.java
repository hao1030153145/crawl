package com.transing.crawl.util.client;

import com.jeeframework.core.exception.BaseException;
import com.jeeframework.logicframework.biz.exception.BizException;
import com.jeeframework.util.httpclient.URLUtil;
...
public class HttpClientHelper
{
	...
	public static final int RESPONSE_RET_STRING = 1; //返回字符串
	public static final int RESPONSE_RET_BYTES = 2; //返回字节
...
//    }
	...
	public HttpResponse doGet(String url, String requestEncoding, String responseEncoding,
		...
		while (true) {
			try {
				return doGetWrapper(url, requestEncoding, responseEncoding, headerMap, proxyIp, RESPONSE_RET_STRING);
			} catch (IOException e) {
				retryTimes++;
	...
	public HttpResponse doGetAndRetBytes(String url, String requestEncoding, String responseEncoding,
		...
		while (true) {
			try {
				return doGetWrapper(url, requestEncoding, responseEncoding, headerMap, proxyIp, RESPONSE_RET_BYTES);
			} catch (IOException e) {
				retryTimes++;
				if (retryTimes == retry_times) {
					throw e;
...
//        }
		...
		try {
			...
			if (!Validate.isEmpty(requestEncoding)) {
				String encodeUrl = getEncodeUrl(url, requestEncoding);
...
//                }
				...
				if (responseEntity != null) {
					...
					try {
						...
						if (responseType == RESPONSE_RET_BYTES) {
							...
							httpResponse.setContentType(
									HttpResponse.CONTENT_TYPE_BYTE);
						} else {
...
//            pairs[i++] = new BasicNameValuePair(entry.getKey(), entry.getValue());
	...
	private static String getHtmlCharset(byte[] htmlBytes, Charset charsetObj) {
		...
		if (charsetObj == null || Validate.isEmpty(charsetObj.name())) {
			...
			String charSetTmp = RegxUtils
					.matchRegxWithPrefix(htmlTmp, regEx, false);
...
//    }
	 ...
	 * @return
     */
	public Header[] getCookieHead(){
		return this.context.getResponse().getHeaders("Set-Cookie");
	}
	/**
	 * 获取返回的cookie
	 * @return
	 */
	public URI getredictURl(){
		return this.context.getRedirectLocations().get(0);
	}
	...
	public static String getEncodeUrl(String urlString, String encode) {
		...
		List<String> list = getParamsMap(queryString, encode);
	...
	private static List<String> getParamsMap(String queryString, String enc) {
